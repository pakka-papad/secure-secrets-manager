...
public class SystemLockService {
  ...
  /**
   * Acquires an exclusive, pessimistic write lock for the given lock name. This method will block
   * for a maximum of 5 seconds (as configured in the repository) until the lock is acquired. If the
   * lock cannot be acquired within this timeout, a {@code PessimisticLockingFailureException} will
   * be thrown, causing the transaction to roll back. The lock is held until the surrounding
   ...
   * @throws org.springframework.dao.PessimisticLockingFailureException if the lock cannot be
  ...
  public void acquireExclusiveLock(SystemLockName lockName) {
    systemLockRepository
        .findAndLockByName(lockName.name())
        .orElseThrow(
            () ->
                new EntityNotFoundException(
                    "System lock with name '" + lockName.name() + "' not found."));
  }
}
